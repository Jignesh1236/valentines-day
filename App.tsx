...
const memories = [
  "Goddess Ji, Main Jab Se Aapko Dekha Hai, Tab Se Dil Mein Something Something Ho Raha Hai 🤌🏻",
  "Madam Ji, Tame Mane Roj Je Selfie Ane Videos Moklo Cho, E Mara Mate Sauthi Special Ane Important Moment Hoy Chhe 😁🤌🏻",
  "Goddess Ji, Tame Mara Mate Bau J Special Cho 😌",
    ...
    <div className="min-h-screen w-screen overflow-x-hidden relative flex items-center justify-center p-2 md:p-4">
      ...
      <div className={`chapter-transition absolute inset-0 flex items-center justify-center ${chapter === 1 ? 'opacity-100 scale-100 z-30' : 'opacity-0 scale-90 -z-10'}`}>
        <div 
          ...
          className="bg-white/80 backdrop-blur-md p-6 md:p-10 rounded-2xl shadow-2xl border-4 border-dashed border-pink-300 text-center cursor-pointer hover:rotate-2 transition-transform group"
        >
          <div className="text-5xl md:text-8xl mb-4 group-hover:scale-110 transition-transform">✉️</div>
          <h1 className="text-xl md:text-3xl font-romantic text-pink-600">Madam Ji, Zara Yeh Open To Kijiye...</h1>
      ...
      <div className={`chapter-transition absolute inset-0 flex flex-col items-center justify-center ${chapter === 2 ? 'opacity-100 translate-y-0 z-20' : 'opacity-0 translate-y-10 -z-10'}`}>
        ...
        <button 
          onClick={nextReason}
          className="mt-6 md:mt-10 bg-pink-500 text-white px-6 md:px-8 py-2 md:py-3 rounded-full font-bold shadow-lg hover:bg-pink-600 transition-colors flex items-center gap-2"
        >
          Aur Batao! <span className="text-xl">👉</span>
        </button>
        <div className="mt-3 md:mt-4 text-pink-400 font-bold text-sm md:text-base">
      ...
      <div className="absolute bottom-2 md:bottom-6 left-0 right-0 text-center text-pink-300 pointer-events-none opacity-40 text-xs md:text-sm">
