    ...
    <div className="h-screen w-screen overflow-hidden relative flex items-center justify-center p-4">
      ...
      <div className={`chapter-transition absolute inset-0 flex items-center justify-center ${chapter === 1 ? 'opacity-100 scale-100 z-30' : 'opacity-0 scale-90 -z-10'}`}>
        <div 
          ...
          className="bg-white/80 backdrop-blur-md p-10 rounded-2xl shadow-2xl border-4 border-dashed border-purple-300 text-center cursor-pointer hover:rotate-2 transition-transform group"
        >
          <div className="text-8xl mb-4 group-hover:scale-110 transition-transform">✉️</div>
          <h1 className="text-3xl font-romantic text-purple-600">Madam Ji, Zara Yeh Open To Kijiye...</h1>
      ...
      <div className={`chapter-transition absolute inset-0 flex flex-col items-center justify-center ${chapter === 2 ? 'opacity-100 translate-y-0 z-20' : 'opacity-0 translate-y-10 -z-10'}`}>
        <div className="bg-white p-6 rounded shadow-2xl transform -rotate-2 max-w-sm w-full border-b-[40px] border-white">
          <div className="bg-purple-100 h-64 w-full flex items-center justify-center rounded overflow-hidden relative">
          ...
          <p className="mt-6 font-romantic text-2xl text-center text-purple-700 leading-relaxed">
            "{memories[reasonIdx]}"
          </p>
        </div>
        <button 
          onClick={nextReason}
          className="mt-10 bg-purple-500 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-purple-600 transition-colors flex items-center gap-2"
        >
      ...
      <div className="absolute bottom-6 left-0 right-0 text-center text-purple-300 pointer-events-none opacity-40">
